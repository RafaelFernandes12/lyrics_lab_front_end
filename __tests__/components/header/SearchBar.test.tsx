...
describe("SearchBar Header component", () => {
  ...
  test("it should render the elements properly", () => {
    render(<SearchBarComponent songs={songs} albums={albums} />);
  ...
  test("the songs and albuns should be links to their detailed pages", () => {
    ...
    expect(screen.getAllByTestId("albumLink")[0]).toHaveAttribute("href", "/album/1");
    expect(screen.getAllByTestId("albumLink")[1]).toHaveAttribute("href", "/album/2");
    expect(screen.getAllByTestId("albumLink")[2]).toHaveAttribute("href", "/album/3");
  });

  test("when typing in the searchBar the search state should be updated", () => {
    render(<SearchBarComponent songs={songs} albums={albums} />);
    fireEvent.click(screen.getByTestId("searchButton"));
    fireEvent.change(screen.getByRole("textbox"), { target: { value: "songName" } });
    expect(screen.getByText("songName")).toBeInTheDocument()
